...
def run_analysis_multiple_periods(shape_name,L,slope,qD,label):
   if shape_name == 'W14X22':
       durations = ['x0.25hr']
   if shape_name == 'W12X22':
        # durations = ['x1hr']
        durations = ['x1hr']
# #    elif shape_name == 'W10X15':
# #         durations = ['x2hr']
   if shape_name == 'W10X22':
        # durations = ['x2hr']
        durations = ['x2hr']
   if shape_name == 'W8X28':
    # durations = ['x2hr']
...
def run_analysis(shape_name, L, slope, qD, label, duration):
    i_variable = np.linspace(1,16,50)
    fragility_data_points_path = f'studies/reliability/Boise Idaho analysis design cases copy 2/100yr-Durations/Fragility_trials_{duration}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        ...
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/Boise Idaho analysis design cases copy 2/100yr-Durations/Fragility_trials_{duration}/{IM}_{1976}.csv'
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Boise Idaho analysis design cases copy 2/100yr-Durations/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Boise Idaho analysis design cases copy 2/100yr-Durations/90_percent_intensity_conf_interval.csv'))
        ...
        plt.savefig(f'studies/reliability/Boise Idaho analysis design cases copy 2/100yr-Durations/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
