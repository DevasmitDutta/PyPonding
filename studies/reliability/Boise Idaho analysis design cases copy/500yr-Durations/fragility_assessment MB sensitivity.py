...
def run_analysis(shape_name, L, slope, qD, label, duration):
    ...
    fragility_data_points_path = f'studies/reliability/Boise Idaho analysis design cases copy/500yr-Durations/Fragility_trials_{duration}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        ...
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/Boise Idaho analysis design cases copy/500yr-Durations/Fragility_trials_{duration}/{IM}_{1976}.csv'
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Boise Idaho analysis design cases copy/500yr-Durations/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Boise Idaho analysis design cases copy/500yr-Durations/90_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc[duration, '500yr']*inch/hr
        perc95['Denver'] = df_95_percentile.loc[duration, '500yr']*inch/hr
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc[duration, '500yr']*inch/hr
        perc95['New York'] = df_95_percentile.loc[duration, '500yr']*inch/hr
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc[duration, '500yr']*inch/hr
        perc95['New Orleans'] = df_95_percentile.loc[duration, '500yr']*inch/hr
        u_6 = norm.ppf(1 - 0.5)
        ...
        plt.savefig(f'studies/reliability/Boise Idaho analysis design cases copy/500yr-Durations/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
