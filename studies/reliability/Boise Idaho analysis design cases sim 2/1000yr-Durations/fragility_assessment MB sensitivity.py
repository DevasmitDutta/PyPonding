...
def run_analysis_multiple_periods(shape_name,L,slope,qD,label):
   if shape_name == 'W10X15':
       durations = ['x0.25hr']
   elif shape_name == 'W8X18':
        # durations = ['x1hr']
        durations = ['x2hr']
#    elif shape_name == 'W10X15':
#         durations = ['x2hr']
   elif shape_name == 'W6X25':
        # durations = ['x2hr']
        durations = ['x1hr']
   elif shape_name == 'W6X20':
       durations = ['x1hr']
   for duration in durations:
    #    SR_ratio = run_analysis(shape_name,L,slope,qD,label,duration)
    #    return SR_ratio
         run_analysis(shape_name,L,slope,qD,label,duration)

def run_analysis(shape_name, L, slope, qD, label, duration):
    i_variable = np.linspace(7,25,50)
    fragility_data_points_path = f'studies/reliability/Melbourne FL analysis design cases/1000yr-Durations/Fragility_trials_{duration}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        ...
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/Melbourne FL analysis design cases/1000yr-Durations/Fragility_trials_{duration}/{IM}_{1976}.csv'
            ...
            with open(output_path, 'w') as output:
                # SR_RAtio = fragility_assessment_copy(shape_name, L, slope, qD, label, IM, output, fragility_data_points, duration)
                # return SR_RAtio
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Melbourne FL analysis design cases/1000yr-Durations/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Melbourne FL analysis design cases/1000yr-Durations/90_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc[duration, '1000yr']*inch/hr
        perc95['Denver'] = df_95_percentile.loc[duration, '1000yr']*inch/hr
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc[duration, '1000yr']*inch/hr
        perc95['New York'] = df_95_percentile.loc[duration, '1000yr']*inch/hr
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc[duration, '1000yr']*inch/hr
        perc95['New Orleans'] = df_95_percentile.loc[duration, '1000yr']*inch/hr
        u_6 = norm.ppf(1 - 0.5)
        ...
        # return ponding_instability_step_with_slope_SR_ratio_for_a_given_z
        exit()    
        ...
        plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/1000yr-Durations/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
