...
for i, duration in enumerate(durations):
    ...
    if duration == 'x0.25hr':
        ...
        data_1 = pd.read_csv(os.path.join(os.getcwd(), f'studies/reliability/Melbourne FL analysis design cases/500yr-Durations/Fragility_trials_{duration}/fragility_data_points1976.csv'))
        ...
        plt.title(f'Fragility Function Fitting (500yr return period)- Melbourne FL  ({duration} duration)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/500yr-Durations/Fragility_fit/fitted_plot_{duration}.png')
...
results_df.to_csv(f'studies/reliability/Melbourne FL analysis design cases/500yr-Durations/Fragility_fit/mu_sigma_opt_durationwise.csv', index=False)

# Add labels and title
plt.xlabel('IM')
plt.ylabel('Probability of Limit State of Exceedance') 
plt.title('Theoritical Fragility Curves - Melbourne FL  (500 yr period)')
plt.legend(loc='best', fontsize=8)
plt.grid(True)
plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/500yr-Durations/Fragility_fit/duration_comparision_plot.png')
