...
for i, duration in enumerate(durations):
    ...
    if duration == 'x0.25hr':
        ...
        data_1 = pd.read_csv(os.path.join(os.getcwd(), f'studies/reliability/Boise Idaho analysis design cases sim/100yr-Durations/Fragility_trials_{duration}/fragility_data_points1976.csv'))
        # print(data_1[duration][~np.isnan(data_1[duration])].values * 200)
        ...
        plt.title(f'Fragility Function Fitting (100yr return period)- Boise Idaho  ({duration} duration)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/Boise Idaho analysis design cases sim/100yr-Durations/Fragility_fit/fitted_plot_{duration}.png')
...
results_df.to_csv(f'studies/reliability/Boise Idaho analysis design cases sim/100yr-Durations/Fragility_fit/mu_sigma_opt_durationwise.csv', index=False)

# Add labels and title
plt.xlabel('IM')
plt.ylabel('Probability of Limit State of Exceedance') 
plt.title('Theoritical Fragility Curves - Boise Idaho  (100 yr period)')
plt.legend(loc='best', fontsize=8)
plt.grid(True)
plt.savefig(f'studies/reliability/Boise Idaho analysis design cases sim/100yr-Durations/Fragility_fit/duration_comparision_plot.png')
