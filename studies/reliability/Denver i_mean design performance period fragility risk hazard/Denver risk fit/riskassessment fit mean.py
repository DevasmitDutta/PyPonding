...
intensity_data = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Denver i_mean design performance period fragility risk hazard/final_precipitation_intensity_data.xlsx - Sheet1.csv'));
...
def duration_window_wise(window_size, duration):
    ...
    for (i, IM) in enumerate(random_samples):
        ...
        df = pd.read_csv(f'studies/reliability/Denver i_mean design performance period fragility risk hazard/Fragility_fit/mu_sigma_opt_durationwise.csv')
        # print(df.values[0])
...
plt.legend()
plt.savefig('studies/reliability/Denver i_mean design performance period fragility risk hazard/Denver risk fit/Probability_of_limit_state_exceedance_X_years_mean.png')