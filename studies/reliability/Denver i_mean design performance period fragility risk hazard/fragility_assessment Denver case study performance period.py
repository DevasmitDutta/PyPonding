...
def run_analysis(shape_name,L,slope,qD,label):
     ...
     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/Denver i_mean design performance period fragility risk hazard/final_precipitation_intensity_data.xlsx - Sheet1.csv'))

    i_variable = np.linspace(4,7,100)

    fragility_data_points = open(f'studies/reliability/Denver i_mean design performance period fragility risk hazard/Fragility_trials/fragility_data_points{1976}.csv','w')


    for IM in i_variable:
        print('intensity',IM)
        output = open(f'studies/reliability/Denver i_mean design performance period fragility risk hazard/Fragility_trials/{IM}_{1976}.csv','w')
        fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points)
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Denver i_mean design performance period fragility risk hazard/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Denver i_mean design performance period fragility risk hazard/95_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc['x0.25hr', '100yr']
        perc95['Denver'] = df_95_percentile.loc['x0.25hr', '100yr']
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc['x0.25hr', '100yr']
        perc95['New York'] = df_95_percentile.loc['x0.25hr', '100yr']
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc['x0.25hr', '100yr']
        perc95['New Orleans'] = df_95_percentile.loc['x0.25hr', '100yr']
        u_6 = norm.ppf(1 - 0.5)
        ...
        plt.savefig(f'studies/reliability/Denver i_mean design performance period fragility risk hazard/Fragility_trials/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
