...
def fit_fragility_curve(window_size):    
        # Define a dark color palette
        dark_colors = list(mcolors.TABLEAU_COLORS.values())[:num_durations]

        # data_2 = pd.read_csv(os.path.join(os.getcwd(), 'studies/reliability/Denver/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
        ...
        for i, duration in enumerate(durations):
            ...
            if duration == 'x1hr':
                ...
                data_1 = pd.read_csv(os.path.join(os.getcwd(), f'studies/reliability/Denver performance period fragility risk hazard/Denver {window_size} performance period/Fragility_trials_{duration}/fragility_data_points1976.csv'))
                ...
                plt.title(f'Fragility Function Fitting (1hr duration)- Denver ({window_size} yr performance period)')
                plt.legend(loc='best', fontsize=8)
                plt.grid(True)
                plt.savefig(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window_size} performance period/Fragility_fit/fitted_plot_1hr.png')
        ...
        results_df.to_csv(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window_size} performance period/Fragility_fit/mu_sigma_opt_durationwise.csv', index=False)

        # Add labels and title
        plt.xlabel('IM')
        plt.ylabel('Probability of Limit State of Exceedance') 
        plt.title(f'Theoritical Fragility Curves - Santa Barbara ({window_size} yr performance period)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window_size} performance period/Fragility_fit/duration_comparision_plot.png')

        # Show the plot
        # plt.show()

for window_size in window_sizes:
    fit_fragility_curve(window_size)
