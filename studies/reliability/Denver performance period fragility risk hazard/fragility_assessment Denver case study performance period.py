...
upper_limit = 0    
# output = open(f'studies/reliability/Denver performance period fragility risk hazard/trials{1976}.csv','w')
window_size = [40] #np.arange(35,41,5)

def run_analysis_different_window_size(shape_name,L,slope,qD,label):
    for window in window_size:
        run_analysis(shape_name,L,slope,qD,label, window)

def run_analysis(shape_name,L,slope,qD,label, window):


     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/Denver performance period fragility risk hazard/final_precipitation_intensity_data.xlsx - Sheet1.csv'))

    # Extract the durations and intensities
    durations = ['x1hr','x2hr','x3hr']
    ...
    for duration in durations:
        ...
        i_variable_2 = np.linspace(0.5,2.5,100)

        fragility_data_points = open(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window} performance period/Fragility_trials_{duration}/fragility_data_points{1976}.csv','w')
        fragility_data_points.write(f'{duration},')  
        fragility_data_points.write('\n')
        ...
        #    break
           print('intensity',IM)
        #    break
           output = open(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window} performance period/Fragility_trials_{duration}/{IM}_{1976}.csv','w')
           fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points, window)
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points, window):
        ...
        rate = {}
        perc95 = {}
        df = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/Denver performance period fragility risk hazard/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
        ...
        plt.savefig(f'studies/reliability/Denver performance period fragility risk hazard/Denver {window} performance period/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
