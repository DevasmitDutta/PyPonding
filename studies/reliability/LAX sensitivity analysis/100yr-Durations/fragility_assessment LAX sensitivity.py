...
def run_analysis(shape_name, L, slope, qD, label, duration):
    ...
    fragility_data_points_path = f'studies/reliability/LAX sensitivity analysis/100yr-Durations/Fragility_trials_{duration}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        ...
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/LAX sensitivity analysis/100yr-Durations/Fragility_trials_{duration}/{IM}_{1976}.csv'
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX sensitivity analysis/100yr-Durations/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX sensitivity analysis/100yr-Durations/95_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc[duration, '100yr']
        perc95['Denver'] = df_95_percentile.loc[duration, '100yr']
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc[duration, '100yr']
        perc95['New York'] = df_95_percentile.loc[duration, '100yr']
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc[duration, '100yr']
        perc95['New Orleans'] = df_95_percentile.loc[duration, '100yr']
        u_6 = norm.ppf(1 - 0.5)
        ...
        for j in range(Ntrials+1):
            ...
            if j == Ntrials:
                meanHV = open(f'studies/reliability/LAX sensitivity analysis/100yr-Durations/Fragility_trials_{duration}/meanHV{label}.csv','w')
                # Ensure the directory for meanHV exists
                os.makedirs(os.path.dirname(f'studies/reliability/LAX sensitivity analysis/100yr-Durations/Fragility_trials_{duration}/meanHV{label}.csv'), exist_ok=True)
        ...
        plt.savefig(f'studies/reliability/LAX sensitivity analysis/100yr-Durations/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
