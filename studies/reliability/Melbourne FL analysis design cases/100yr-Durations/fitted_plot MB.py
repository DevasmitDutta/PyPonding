...
for i, duration in enumerate(durations):
    ...
    if duration == 'x0.25hr':
        ...
        data_1 = pd.read_csv(
        os.path.join(os.getcwd(), f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_trials_{duration}/fragility_data_points1976.csv'),
        usecols=[f'{duration}', 'intensity']
    )
        ...
        # IM = intensities
        IM = data_1['intensity'][~np.isnan(data_1['intensity'])].values
        ...
        IM_plot = data_1['intensity'][~np.isnan(data_1['intensity'])].values #np.linspace(7,25,100)
        ...
        plt.title(f'Fragility Function Fitting (100yr return period)- Melbourne FL  ({duration} duration)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_fit/fitted_plot_{duration}.png')
...
for i, duration in enumerate(durations):
    ...
    # Plot the fitted fragility curve
    IM_plot = data_1['intensity'][~np.isnan(data_1['intensity'])].values
    p_fit = norm.cdf(np.log(IM_plot), loc=mu_opt, scale=sigma_opt)
    plt.plot(IM_plot, p_fit, color=dark_colors[i], linestyle='--', label=f'Fitted Lognormal CDF ({duration})\nmu={mu_opt:.3f}, sigma={sigma_opt:.3f}')

# Save the results DataFrame to a CSV file
results_df.to_csv(f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_fit/mu_sigma_opt_durationwise.csv', index=False)

# Add labels and title
plt.xlabel('IM')
plt.ylabel('Probability of Limit State of Exceedance') 
plt.title('Theoritical Fragility Curves - Melbourne FL  (100 yr period)')
plt.legend(loc='best', fontsize=8)
plt.grid(True)
plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_fit/duration_comparision_plot.png')
