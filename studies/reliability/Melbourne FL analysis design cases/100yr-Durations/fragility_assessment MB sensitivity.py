...
def run_analysis_multiple_periods(shape_name,L,slope,qD,label):
   if shape_name == 'W14X22':
       durations = ['x0.25hr']
   elif shape_name == 'W10X12':
        # durations = ['x1hr']
        durations = ['x1hr']
#    elif shape_name == 'W10X15':
#         durations = ['x2hr']
   elif shape_name == 'W12X14':
        # durations = ['x2hr']
        durations = ['x2hr']
...
def run_analysis(shape_name, L, slope, qD, label, duration):
    if shape_name == 'W6X25':
       i_variable = np.linspace(15,30,50)
    else: 
        i_variable = np.linspace(7,25,50)

    fragility_data_points_path = f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_trials_{duration}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        fragility_data_points.write(f'{duration},intensity')  
        fragility_data_points.write('\n')
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_trials_{duration}/{IM}_{1976}.csv'
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/90_percent_intensity_conf_interval.csv'))
        ...
        pf = cnt_fail/Ntrials
        fragility_data_points.write(f'{pf},{IM},\n')
        ...
        plt.savefig(f'studies/reliability/Melbourne FL analysis design cases/100yr-Durations/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
