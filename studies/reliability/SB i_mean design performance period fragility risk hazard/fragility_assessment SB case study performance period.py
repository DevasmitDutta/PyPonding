...
def run_analysis(shape_name,L,slope,qD,label):
     ...
     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/SB i_mean design performance period fragility risk hazard/final_precipitation_intensity_data.xlsx - Sheet1.csv'))

    i_variable = np.linspace(1,4,100)

    fragility_data_points = open(f'studies/reliability/SB i_mean design performance period fragility risk hazard/Fragility_trials/fragility_data_points{1976}.csv','w')


    for IM in i_variable:
        print('intensity',IM)
        output = open(f'studies/reliability/SB i_mean design performance period fragility risk hazard/Fragility_trials/{IM}_{1976}.csv','w')
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB return period fragility risk hazard/SB 100/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB return period fragility risk hazard/SB 100/95_percent_intensity_conf_interval.csv'))
        ...
        plt.savefig(f'studies/reliability/SB i_mean design performance period fragility risk hazard/Fragility_trials/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
