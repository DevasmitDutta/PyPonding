...
def duration_window_wise(window_size, duration):
    ...
    num_samples = 1000
    mean_data = pd.read_csv(f'studies/reliability/SB performance period fragility risk hazard/mean_intensity_conf_interval.csv')
    _95thpercentile_data = pd.read_csv(f'studies/reliability/SB performance period fragility risk hazard/95_percent_intensity_conf_interval.csv')
...
for (i, duration) in enumerate(columns):
    ...
    plt.legend()
    plt.savefig(f'studies/reliability/SB performance period fragility risk hazard/SB risk conf interval/Random_samples_from_GEV_distribution_{duration}.png')    
...
for (i, duration) in enumerate(columns):
    ...
    for (j, window_size) in enumerate(window_sizes):
        ...
        plt.ylabel('Frequency')
        plt.legend()
        plt.savefig(f'studies/reliability/SB performance period fragility risk hazard/SB risk conf interval/Random_samples_from_GEV_distribution_{window_size}_yr_{duration}.png')    