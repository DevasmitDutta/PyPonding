...
window_sizes = [2, 5, 10, 25] #np.arange(5,41,1) #[5, 8, 10, 15, 20, 30, 40, 50, 60]  # Generates values from 1 to 10 with a step size of 1
...
def duration_window_wise(window_size, duration):
    ...
    random_gev_params = gev_params[np.random.randint(0, len(gev_params))]
        # Assuming mean_gev_params_1 contains the shape, location, and scale parameters
    shape, loc, scale = random_gev_params
    ...
    for (i, IM) in enumerate(random_samples):
        ...
        df = pd.read_csv(f'studies/reliability/SB {window_size}/Fragility_fit/mu_sigma_opt_durationwise.csv')
...
plt.legend()
plt.savefig('studies/reliability/SB risk fit/Probability_of_limit_state_exceedance_X_years_random_block.png')