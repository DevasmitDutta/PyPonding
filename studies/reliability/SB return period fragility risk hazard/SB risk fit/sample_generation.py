...
for (i, duration) in enumerate(columns):
    ...
    plt.legend()
    plt.savefig(f'studies/reliability/SB risk fit/Random_samples_from_GEV_distribution_{duration}.png')    
...
for (i, duration) in enumerate(columns):
    ...
    for (j, window_size) in enumerate(window_sizes):
        ...
        plt.ylabel('Frequency')
        plt.legend()
        plt.savefig(f'studies/reliability/SB risk fit/Random_samples_from_GEV_distribution_{window_size}_yr_{duration}.png')    