...
intensity_data = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB/final_precipitation_intensity_data.xlsx - Sheet1.csv'));
...
window_sizes = np.arange(5,41,1) #[5, 8, 10, 15, 20, 30, 40, 50, 60]  # Generates values from 1 to 10 with a step size of 1
line_styles = ['-', '--', ':', '-.', '.-.']  # Different line styles for each window size

columns = ['x0.25hr', 'x0.50hr', 'x0.75hr', 'x1hr', 'x2hr', 'x3hr']
...
def duration_window_wise(window_size, duration):
    ...
    for (i, IM) in enumerate(random_samples):
        ...
        df = pd.read_csv('studies/reliability/SB/Fragility_fit/mu_sigma_opt_durationwise.csv')
...
plt.legend()
plt.savefig('studies/reliability/SB/Fragility_fit/Probability_of_limit_state_exceedance_X_years.png')