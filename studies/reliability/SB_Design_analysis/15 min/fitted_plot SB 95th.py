...
def filter_by_percentile_im(IM, num_gms, num_collapse, percentile=95, tol=1e-6):
    """
    Keep only one representative (percentile IM) per probability level,
    ...
    for i in range(len(df)):
        ...
        group = df[mask]
        used[mask] = True
        perc_im = np.percentile(group['IM'], percentile)
        closest_row = group.iloc[(group['IM'] - perc_im).abs().argmin()]
...
def fn_mle_pc(IM, num_gms, num_collapse, filter_data=True):
    if filter_data:
        IM, num_gms, num_collapse = filter_by_percentile_im(IM, num_gms, num_collapse, percentile=95)
    ...
    for j, shape in enumerate(shape_name):
        ...
        data_1 = pd.read_csv(
            os.path.join(os.getcwd(),
                         f'studies/reliability/LAX_Design_analysis/15 min/Fragility_trials_{duration}_{shape_name[j]}/fragility_data_points1976.csv')
        )
        ...
        out_dir = f'studies/reliability/LAX_Design_analysis/15 min/Fragility_fit_95th_{duration}_{shape_name[j]}'
        os.makedirs(out_dir, exist_ok=True)

        plt.title(f'Fragility Function Fitting (100yr return period) - LAX ({duration} duration)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/LAX_Design_analysis/15 min/Fragility_fit_95th_{duration}_{shape_name[j]}/fitted_plot.png')
        plt.show()

        print(f"Duration: {duration}, Shape: {shape}")
