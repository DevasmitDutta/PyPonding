...
for i, duration in enumerate(durations):
    for j, shape in enumerate(shape_name):
        ...
        data_1 = pd.read_csv(os.path.join(os.getcwd(), f'studies/reliability/SB_Design_analysis/2_hr/Fragility_trials_{duration}_{shape_name[j]}/fragility_data_points1976.csv'))
        ...
        out_dir = f'studies/reliability/SB_Design_analysis/2_hr/Fragility_fit_{duration}_{shape_name[j]}'
        os.makedirs(out_dir, exist_ok=True)

        plt.savefig(f'{out_dir}/fitted_plot.png')
        plt.savefig(f'studies/reliability/SB_Design_analysis/2_hr/Fragility_fit_{duration}_{shape_name[j]}/fitted_plot.png')
        plt.show()
