...
for i, duration in enumerate(durations):
    for j, shape in enumerate(shape_name):
        ...
        out_dir = f'studies/reliability/LAX_Design_analysis/15 min/Fragility_fit_1000sampmedian_{duration}_{shape_name[j]}'
        os.makedirs(out_dir, exist_ok=True)

        plt.title(f'Fragility Function Fitting (100yr return period) - LAX ({duration} duration)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)
        plt.savefig(f'studies/reliability/LAX_Design_analysis/15 min/Fragility_fit_1000sampmedian_{duration}_{shape_name[j]}/fitted_plot.png')
        plt.show()
