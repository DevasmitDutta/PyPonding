...
shape_name = ['W14X26','W12X19','W8X18']
...
for i, duration in enumerate(durations):
    ...
    #    for param_comb in ['0.5_1_1','0.25_1_1','0.75_1_1','1_0.5_1','1_0.25_1','1_0.75_1','1_1_1.5','1_1_1.25','1_1_2']:
      for param_comb in ['1_1_2.5','1_1_3','1_1_3.5']: 
        # Create a new figure for each duration and shape
        plt.figure(figsize=(20, 12))

        # Load the data from the CSV file
        data_1 = pd.read_csv(os.path.join(os.getcwd(), f'studies/reliability/SB_sens_analysis/15 min/Fragility_trials_{duration}_{shape}_{param_comb}/fragility_data_points1976.csv'))
        ...
        plt.title(f'Fragility Function Fitting ({shape})- SB ({duration} return period)')
        plt.legend(loc='best', fontsize=8)
        plt.grid(True)

        # Ensure output directory exists
        out_dir = f'studies/reliability/SB_sens_analysis/15 min/Fragility_fit_{duration}_{shape}_{param_comb}'
        os.makedirs(out_dir, exist_ok=True)

        plt.savefig(f'{out_dir}/fitted_plot.png')
        plt.savefig(f'studies/reliability/SB_sens_analysis/15 min/Fragility_fit_{duration}_{shape}_{param_comb}/fitted_plot.png')
        plt.show()
