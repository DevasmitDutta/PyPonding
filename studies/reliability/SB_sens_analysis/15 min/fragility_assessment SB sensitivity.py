...
from math import gamma, pi,cos,cosh,ceil,log
...
def run_analysis_multiple_periods(shape_name,L,slope,qD,label, beta, gamm, alpha):
   for duration in durations:
       run_analysis(shape_name,L,slope,qD,label,duration, beta, gamm, alpha)

def run_analysis(shape_name, L, slope, qD, label, duration, beta, gamm, alpha):
    i_variable = np.linspace(1,16,50) #np.arange(1,16) #np.linspace(3,9,50) #np.linspace(10, 50, 50)
    fragility_data_points_path = f'studies/reliability/SB_sens_analysis/15 min/Fragility_trials_{duration}_{shape_name}_{beta}_{gamm}_{alpha}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        ...
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/SB_sens_analysis/15 min/Fragility_trials_{duration}_{shape_name}_{beta}_{gamm}_{alpha}/{IM}_{1976}.csv'
            ...
            with open(output_path, 'w') as output:
                fragility_assessment_copy(shape_name, L, slope, qD, label, IM, output, fragility_data_points, duration, beta, gamm, alpha)
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, duration, beta, gamm, alpha):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/SB_sens_analysis/15 min/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),f'studies/reliability/SB_sens_analysis/15 min/95_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc['x0.25hr',duration]*inch/hr
        perc95['Denver'] = df_95_percentile.loc['x0.25hr',duration]*inch/hr
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc['x0.25hr',duration]*inch/hr
        perc95['New York'] = df_95_percentile.loc['x0.25hr',duration]*inch/hr
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc['x0.25hr',duration]*inch/hr
        perc95['New Orleans'] = df_95_percentile.loc['x0.25hr',duration]*inch/hr
        u_6 = norm.ppf(1 - 0.5)
        ...
        print('zw_lim for no slope',zw_lim_1,'\n')
        ...
        print('zw_lim for slope',zw_lim_3,'\n')
        design_z = 2 + (1.5*(rate['Denver']*As)/(cd*ws*(2*g)**0.5))**(2.0/3) #3.14
        ...
        plt.savefig(f'studies/reliability/SB_sens_analysis/15 min/Fragility_trials_{duration}_{shape_name}_{beta}_{gamm}_{alpha}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
