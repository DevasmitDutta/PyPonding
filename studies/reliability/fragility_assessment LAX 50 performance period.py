...
def run_analysis(shape_name,L,slope,qD,label, window):
     ...
     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX 50 performance period/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
    ...
    for duration in durations:
        ...
        # factor = X_bar - Y_bar
        i_variable_2 = np.linspace(0.5,3.5,100)
        ...
        # break
        fragility_data_points = open(f'studies/reliability/LAX 50 performance period/Fragility_trials_{duration}/fragility_data_points{1976}.csv','w')
        fragility_data_points.write(f'{duration},')  
        fragility_data_points.write('\n')
        ...
        #    break
        #    print('intensity',IM)
           output = open(f'studies/reliability/LAX 50 performance period/Fragility_trials_{duration}/{IM}_{1976}.csv','w')
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points, window):
        ...
        rate = {}
        perc95 = {}
        df = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX 50 performance period/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
        # intensity = df[duration][~np.isnan(df[duration])].values[:50]
        ...
        plt.savefig(f'studies/reliability/LAX 50 performance period/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
