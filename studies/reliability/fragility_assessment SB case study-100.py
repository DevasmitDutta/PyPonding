...
def run_analysis(shape_name,L,slope,qD,label):
     ...
     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB 100/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
    ...
    for duration in durations:
        ...
        i_variable_2 = np.linspace(2.5,4.5,100)

        fragility_data_points = open(f'studies/reliability/SB 100/Fragility_trials_{duration}/fragility_data_points{1976}.csv','w')
        fragility_data_points.write(f'{duration},')  
        fragility_data_points.write('\n')
        ...
        #    break
           print('intensity',IM)
        #    break
           output = open(f'studies/reliability/SB 100/Fragility_trials_{duration}/{IM}_{1976}.csv','w')
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points):
        ...
        rate = {}
        perc95 = {}
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB 100/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB 100/95_percent_intensity_conf_interval.csv'))
        ...
        plt.savefig(f'studies/reliability/SB 100/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
