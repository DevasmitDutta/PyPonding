...
def run_analysis(shape_name,L,slope,qD,label):
     ...
     # Load the dataset
    data = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
    ...
    for duration in durations:
        ...
        #    break
           print('intensity',IM)
        #    break
           output = open(f'studies/reliability/SB/Fragility_trials_{duration}/{IM}_{1976}.csv','w')
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points):
        ...
        rate = {}
        perc95 = {}
        df = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/SB/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
        ...
        plt.savefig(f'studies/reliability/SB/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
