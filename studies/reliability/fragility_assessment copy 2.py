...
def run_analysis(shape_name,L,slope,qD,label):
    ...
    for duration in durations:
        ...
        fragility_data_points = open(f'studies/reliability/Fragility_trials_{duration}/fragility_data_points{1976}.csv','w')
        fragility_data_points.write(f'{duration},')  
        fragility_data_points.write('\n')
        ...
        #    break
           print('intensity',IM)
        #    break
           output = open(f'studies/reliability/Fragility_trials_{duration}/{IM}_{1976}.csv','w')
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, duration, fragility_data_points):
        ...
        rate = {}
        perc95 = {}
        df = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/final_precipitation_intensity_data.xlsx - Sheet1.csv'))
        intensity = df['x2hr']
        ...
        plt.savefig(f'studies/reliability/Fragility_trials_{duration}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
