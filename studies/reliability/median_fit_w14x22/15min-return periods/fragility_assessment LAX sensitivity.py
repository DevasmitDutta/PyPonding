...
def run_analysis_multiple_periods(shape_name,L,slope,qD,label):
   for period in periods:
       run_analysis(shape_name,L,slope,qD,label,period)

def run_analysis(shape_name, L, slope, qD, label, period):
    i_variable = np.linspace(1, 5, 50)
    fragility_data_points_path = f'studies/reliability/LAX sensitivity analysis/15min-return periods/Fragility_trials_{period}/fragility_data_points{1976}.csv'
    ...
    with open(fragility_data_points_path, 'w') as fragility_data_points:
        fragility_data_points.write(f'{period},')  
        fragility_data_points.write('\n')
        for IM in i_variable:
            print('intensity', IM)
            output_path = f'studies/reliability/LAX sensitivity analysis/15min-return periods/Fragility_trials_{period}/{IM}_{1976}.csv'
            ...
            with open(output_path, 'w') as output:
                fragility_assessment_copy(shape_name, L, slope, qD, label, IM, output, fragility_data_points, period)
...
def fragility_assessment_copy(shape_name,L,slope,qD,label, IM, output, fragility_data_points, period):
        ...
        df_mean = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX sensitivity analysis/15min-return periods/mean_intensity_conf_interval.csv'))
        df_mean.set_index('Dur (hr)', inplace=True)

        df_95_percentile = pd.read_csv(os.path.join(os.getcwd(),'studies/reliability/LAX sensitivity analysis/15min-return periods/95_percent_intensity_conf_interval.csv'))
        ...
        rate['Denver'] = df_mean.loc['x0.25hr', period]
        perc95['Denver'] = df_95_percentile.loc['x0.25hr', period]
        u_4 = norm.ppf(1 - 0.5)
        rate['New York'] = df_mean.loc['x0.25hr', period]
        perc95['New York'] = df_95_percentile.loc['x0.25hr', period]
        u_5 = norm.ppf(1 - 0.5)
        rate['New Orleans'] = df_mean.loc['x0.25hr', period]
        perc95['New Orleans'] = df_95_percentile.loc['x0.25hr', period]
        u_6 = norm.ppf(1 - 0.5)
        ...
        plt.savefig(f'studies/reliability/LAX sensitivity analysis/15min-return periods/Fragility_trials_{period}/{IM}_1976.png', bbox_inches='tight')   
        # plt.show()
